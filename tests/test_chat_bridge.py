...
async def test_consume_game_chat_drops_bot_producer(settings, mock_redis):
    ...
    mock_redis.xreadgroup.side_effect = [mock_redis.xreadgroup.return_value, KeyboardInterrupt("Stop loop")]
    ...
    mock_redis.xack.assert_called_once_with(b"xcore:evt:chat:message", f"{settings.redis_group_prefix}:discord-chat", b"123-0")
...
async def test_consume_game_chat_allows_server_producer(settings, mock_redis):
    ...
    callback.assert_called_once_with(GameChatMessage(author_name="Player", message="Hi", server="test-server"))
    mock_redis.xack.assert_called_once_with(b"xcore:evt:chat:message", f"{settings.redis_group_prefix}:discord-chat", b"123-0")
